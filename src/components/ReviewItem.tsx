  ...
  return (
    ...
    >
      <div className=" flex">
        ...
        <div className="flex-1 flex justify-between mr-4">
          <div className="text-sm sm:text-base">
            <span className="block font-semibold dark:text-typoPrimaryColor">{data.name}</span>
            <span className="block mt-0.5 text-slate-500 dark:text-darkGray4 text-sm">
              {data.date}
            </span>
          </div>
      ...
      <div className="mt-4 prose prose-sm sm:prose dark:prose-invert sm:max-w-2xl">
        <p className="text-slate-600 dark:text-typoPrimaryColor">{data.comment}</p>
      </div>
    </div>
  );
};
